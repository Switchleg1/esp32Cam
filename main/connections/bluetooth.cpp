...
uint16_t CBluetooth::sendBle(CPacket* packet)
{
    ...
    while(packet->size()) {
        uint16_t sendLength = packet->size();
        ...
        uint8_t timeout = 0;
        while (esp_ble_gatts_send_indicate(Bluetooth.bleSppGattsIf, Bluetooth.bleSppConnId, Bluetooth.sppHandleTable[SPP_IDX_SPP_DATA_NTY_VAL], sendLength, packet->data(), false) != ESP_OK && timeout++ < 10) {
            ESP_LOGW(CBT_TAG, "sendBle: write failure");
            retValue = BT_WRITE_FAILURE;
            vTaskDelay(pdMS_TO_TICKS(Bluetooth.sendDelay));
        }

        packet->forward(sendLength);
